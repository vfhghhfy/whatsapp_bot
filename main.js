...
require('./config.js');
console.log('🔄 [DEBUG] Main.js loaded - Version 2.0 - Song command REMOVED');
...
async function handleMessages(sock, messageUpdate, printLog) {
    let chatId; // Define chatId outside try block
    try {
        ...
        const adminCommands = ['.mute', '.unmute', '.ban', '.unban', '.promote', '.demote', '.kick', '.tagall', '.antilink', '.reminder'];
        ...
        switch (true) {
            ...
            case userMessage === '.meme':
                ...
                break;
            case userMessage === '.proxy':
                await proxyCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.tempmail'):
                const tempmailArgs = userMessage.split(' ').slice(1);
                await tempmailCommand(sock, chatId, message, tempmailArgs);
                break;
            case userMessage.startsWith('.reminder'):
                // For reminder commands, get admin status if in group
                let isAdminForReminder = false;
                let groupMetaForReminder = null;

                if (isGroup) {
                    const adminStatus = await isAdmin(sock, chatId, senderId, message);
                    isAdminForReminder = adminStatus.isSenderAdmin;
                    try {
                        groupMetaForReminder = await sock.groupMetadata(chatId);
                    } catch (error) {
                        console.error('Error getting group metadata for reminder:', error);
                    }
                }
                ...
                await reminderCommand.execute(sock, message, reminderArgs, {
                    isGroup,
                    isAdmin: isAdminForReminder,
                    groupMetadata: groupMetaForReminder,
                    pushName,
                    sender: senderId
                });
                break;
            case userMessage.startsWith('.google'):
                ...
                await googleCommand.execute(sock, message, googleArgs);
                break;
            case userMessage.startsWith('.apk'):
                const apkArgs = userMessage.split(' ').slice(1);
                await apkCommand.execute(sock, message, apkArgs);
                break;
            case userMessage.startsWith('.playstore'):
                const playstoreArgs = userMessage.split(' ').slice(1);
                await playstoreCommand.execute(sock, message, playstoreArgs);
                break;
            case userMessage.startsWith('.ask'):
            ...
            case userMessage.startsWith('.play') || userMessage.startsWith('.mp3') || userMessage.startsWith('.ytmp3'):
                console.log('🎵 [DEBUG] Routing .play command to playCommand - Updated main.js');
                await playCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.tiktok') || userMessage.startsWith('.tt'):
                ...
                break;
            case userMessage.startsWith('.gpt') || userMessage.startsWith('.gemini') || userMessage.startsWith('.grok'):
...
module.exports = {
    ...
    initializeBot: (sock) => {
        ...
        if (reminderCommand.init) {
            reminderCommand.init(sock);
        }
